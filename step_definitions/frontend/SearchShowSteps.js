const { I , searchShowPage , loginPage , logoutPage}   = inject();
...
Then('I click on the shows button on home page', async function() {
  I.wait(3); 
  await searchShowPage.clickOnShowsTab();
});

When('I play any show randomly for {int} seconds and go back', async function(seconds) {
  await searchShowPage.playRandomShowAndWatch(seconds);
});
